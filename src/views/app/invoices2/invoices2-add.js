//TODO: check what if we put some undividable numbers (make number rounding work)
// TODO: Discount, tax value, total etc make .00
// TODO: import partial payments from silver
...
export default class Invoices2Add extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      date: moment(),
      selectedCustomer: { label: null, value: null },
  ...
  componentDidMount = async () => {
    // If customerid setup in URL preset the added customer
    let customerid = this.props.match.params.customerid; 
    if (customerid){
      console.log(`Invoices2Add -> componentDidMount -> customerid`, customerid);
      this.setCustomerById(customerid);
    } else {
      this.Invoices2.loadProducts();
    }
  }

  setCustomerById = async (id, executeAfter) => {
    let selectedCustomer = {label: "Loading...",value: ""};
    await this.setState({selectedCustomer});
    this.Invoices2.loadCustomerById(id, async (e,customerData)=>{
      if(e){ console.log(`Invoices2Add -> setCustomerById -> Error: `, e); return;};
      let newItem = this.state.item;
      newItem['refCode'] = customerData.referrer;
      await this.setState({ selectedCustomer: {label: customerData.name, value: customerData._id }, item: newItem });
      //this.handleCustomerChange({'label': customerData.name, 'value':customerData._id, 'details': customerData});
    })
  }
  ...
  handleDateChange = async (date) => {
    let newItem = this.state.item;
    newItem['date'] = date?date.format("YYYY-MM-DD"):'';
  ...
  handleAddProduct = async () => {
    ...
    console.log("Invoices2Add -> handleAddProduct= -> currentProduct", currentProduct);
  ...
  save = async () => {
    ...
    this.Invoices2.addItem(this.state.item);
  }
  ...
  render() {
    ...
    return (
      <Fragment>
        <Row>
          <Colxx xxs="12">
            <Breadcrumb heading="survey.add-new" match={this.props.match} />
                ...
                <div className="mb-5">
                  <div className="d-flex flex-row align-items-center mb-3">
                    ...
                    <div className="pl-3 pt-2 pr-2 pb-2">
                      <p className="list-item-heading mb-1">Invoice</p>
                    </div>
                  </div>
                </div>
                <Form>
                  <FormGroup row>
                    <Colxx sm={4}>
                      <FormGroup>
                        <CustomersAsync customerId={this.state.selectedCustomer.value} customerChange={this.handleCustomerChange} />
                      </FormGroup>
                    </Colxx>
                    <Colxx sm={4}>
                      <FormGroup>
              ...
              <CardBody>
                <Form>
                  <FormGroup row>
                    ...
                    <Colxx sm={4} style={{marginTop: '10px', marginBottom: '10px'}}>
                      <div style={{textAlign: 'left'}}>Discount: {this.state.item.discount?this.state.item.discount:0} %</div>
                      <div style={{textAlign: 'left', fontWeight: 'bold'}}>Total: {this.state.item.total} {this.state.currencySymbol}</div>
                      <div style={{textAlign: 'left'}}>Tax value: {this.state.item.taxTotal} {this.state.currencySymbol}</div>
                      <div style={{textAlign: 'left'}}>Refereal code: "{this.state.item.refCode}"</div>
                    </Colxx>
